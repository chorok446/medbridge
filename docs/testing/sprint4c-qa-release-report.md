...
> **출시 판정: 보류 (HOLD)** — 2026-09-14 고정 합성 출처 선택 회귀 게이트를 추가했다(앱 미연결).
> 최종 실제 모델 9회에서 기준 누락 3회·무관 자료 오선택 3회를 검출하고 종료 코드 2로 실패시켰다.
> 평가의 실패 검출은 확인했지만 선택 품질은 개선/통과되지 않았다. 기존 선택기·제품은 그대로다.
> 새 단위 55개, 관련 124개와 Ruff·mypy 통과. 최초 중단 포함 선택기 호출 시도 16회, DB 쓰기 0회다.
> 최종 전체 API **1,793 passed / 4 skipped**, 132.97초. 자동 테스트 통과와 선택 품질 실패를 구분한다.
