...
> **출시 판정: 보류 (HOLD)** — 2026-09-10 `088226f`에서 분류 후속 근거 검색과
> 전체 출처 누락의 1회 재요청을 보강했다. API 1,362개 통과지만 고정 전체 모델 평가는
> **38/39, release_hold**: 원문 밖 설명 노출 1건이다. 실제 분류 질문 6회 중 4회는
> 서버 상충 판정이고 NYHA 표지는 지원 주장에 없다. 완전 해결이나 설치본 검증이 아니다.
> 현재 설치본 `7234601`에는 후속 수정이 없다. 전체 Windows·대형 업로드/메모리도 남는다.
