...
> **출시 판정: 보류 (HOLD)** — 2026-09-09 새 설치본 검증과 취소 오분류 수정
> `d564828` CI 설치본에서 긴 질문 3/3과 중복 제거·출처 이동은 통과했지만
> 중단 버튼이 접수된 취소를 CONNECTION_LOST로 저장했다. `0fc7eae`에서 수정했고
> API 1,185개가 통과했다. 수정본 설치·전체 모델 재평가와 Windows 36항목은 남는다.
