...
> **출시 판정: 보류 (HOLD)** — 2026-09-12 `5ef7e14`은 검증된 절 경계의 청크 전달이다.
> 새 회귀 **29개**, 전체 **1,549 passed / 4 skipped**. Ruff·mypy 통과.
> 실제 문제 페이지의 메모리 비교에서 이전 표와 새 절을 분리하고 네 항목을 함께 유지했다.
> 사용자 데이터·설치본은 그대로이며 전체 검색·최종 답변의 분류 완결성은 미해결이다.
> 평가용 원문 선택 시험은 계속 앱 미연결이다.
> 직전 제품의 고정 모델 평가 **39/39**는 새 코드에 대해 재실행하지 않았다. GUI도 후속이다.
> 전체 Windows·깨끗한 PC·대형 신규 업로드/메모리 검증도 남는다.
> PR Draft 유지, Ready·병합·릴리스·승인 파일 생성 없음.

## 2026-09-12 검증된 절 경계의 청크 전달 — 5ef7e14

- 기존 추출 배치 조건을 통과한 두 줄 번호 절만 JSON 경계 표시를 저장한다.
  청크 생성기는 디지털 TEXT의 boolean true만 사용하고 제목이 같아도 경계를 보존한다.
- 새 회귀 **29개**, 기존 추출·검색 포함 **130 passed**. 저장 후 새 세션에서 읽어
  실제 청크 재생성을 두 번 검증했다. 원문·출처·좌표·순서 및 중복 방지를 확인했다.
- 경계 소비만 끈 대조군은 **4 failed / 25 passed**로 분리 오류를 검출했다.
  정상 제품 전체 **1,549 passed / 4 skipped**, 124.06초. Ruff·mypy 131개 소스 통과다.
  직전 작업의 연결 오류는 이번 전체 검사에서 발생하지 않았다. 네트워크 코드는 그대로다.
- 실제 두 페이지 79블록을 읽기 전용으로 비교했다. 문제 페이지 단독 청크는 10→11개,
  이전 표/새 절이 분리되고 네 항목은 함께 남는다. 인접 표 캡션/네 행 연결은 유지한다.
- 사용자 문서·설정·대화 해시가 같으며 설치·원본 재추출·활성 청크 교체·모델 호출은 없다.
  기존 메타데이터에는 경계가 없어 청크 재생성만으로 적용되지 않는다.
  다음은 격리 사본의 전체 청크/동일 질문 비교이며, 고정 모델 평가·GUI도 후속이다.
  [재현·저장 경로 검증·실제 비교·한계](qa-section-chunk-boundary-20260912.md).

## 2026-09-12 번호 절 제목 읽기 순서 — df67118

- 번호가 첫 줄에 단독으로 추출된 짧은 절 제목에만 위 여백·겹침 부재·직후 전폭
  본문을 요구한다. 일반 목록/한 줄 번호/로마 숫자 전체로 확대하지 않는다.
- 새 회귀 **29 passed**, 기존 추출 포함 **79 passed**. 절 판단을 끈 대조군은
  **7 failed / 45 passed**이며 기존 표 캡션 23개는 그대로 통과한다.
- 첫 전체 실행은 **1 failed / 1,519 passed / 4 skipped**로 연결 오류 1건이 있었다.
  단독 1개와 관련 81개는 통과했고, 오류 종류를 기록하는 계측 전체 재검증은
  **1,520 passed / 4 skipped**, 148.14초다. 원인은 미확정이며 네트워크 코드는 그대로다.
  Ruff·mypy 131개 소스·diff check도 통과했다.
- 읽기 전용 실제 좌표 재계산에서 제목과 선행 내용 두 블록의 순서를 보정했다.
  신뢰도 0.5와 원문·좌표·출처 reading_order 정합성 및 사용자 데이터 해시를 유지한다.
- 해당 페이지 단독 초안은 여전히 절 제목과 이전 표를 같은 청크에 넣는다.
  청크 생성기가 줄바꿈 있는 제목을 거부하기 때문이다. 다음은 검증된 절 경계의
  청크 분리 전달이며, 전체 검색·답변·고정 모델 평가와 새 설치본 검증은 후속이다.
  [재현·실패·읽기 전용 측정·한계](qa-section-order-20260912.md).

## 2026-09-12 표 캡션 경계 읽기 순서 — 6910217

- 2단 본문의 짧은 번호 표 제목과 인접 전폭 행이 확인되는 경계만 보정한다.
  기존 구간 내부 열 순서·원문·좌표를 보존하고 보정 신뢰도를 최대 0.5로 제한한다.
- 새 합성 회귀 **23 passed**, 기존 추출 포함 **50 passed**. 페이지/블록/줄 준비에서
  청크 초안의 출처까지 확인했다. 경계를 끈 대조군은 **9 failed / 14 passed**다.
  전체 API **1,491 passed / 4 skipped**, 119.68초, Ruff·mypy 131개 소스 통과.
- 관련 두 페이지 79블록을 읽기 전용으로 재계산했다. 한 표의 제목과 네 행은 연속으로
  묶이지만 일반 번호 절 제목 오류는 남는다. 해당 페이지 단독 청크 초안 비교이며
  전체 문서의 활성 청크·검색·실제 답변이 개선됐다는 검증은 아니다.
- 문서·설정·대화의 전후 해시가 같고 DB 무결성 정상이다. 자동 재추출·설치·모델 호출 없음.
  다음은 격리된 자료 사본에서 나머지 경계와 전체 청크/동일 질문을 비교하는 일이다.
  새 코드의 고정 모델 평가와 설치본 검증 전까지 HOLD다.
  [원인·재현·한계·다음 작업](qa-table-caption-order-20260912.md).

## 2026-09-12 평가용 원문 선택 시험 구현 — a08b077

- 모델이 문장을 재작성하지 않고 모든 입력 번호의 포함/제외를 반환한다.
  서버는 가시 원문·기존 ID·출처·해시를 복사한다. 누락·중복·미상 키와 추가 산문,
  입력 변경·절단·취소·시간 초과를 거부하며 네트워크 요청 1회로 제한한다.
- 실제 원래 이력의 두 질문으로 목록 6회, 배열 6회, 배열 계측 2회,
  필수 판단 객체 6회, 최종 모듈 2회 확인을 수행했다. 같은 모델/입력/사용자 DB다.
  목록은 1개 선택, 배열은 판단 누락으로 거부됐다. 필수 객체도 1개/2개 청크만 골랐다.
  원문 보존은 확인했지만 HFpEF/NYHA 근거 누락이 남아 제품에 적용하지 않았다.
- 새 단위 **57 passed**, 기존 수치 경계 포함 **65 passed**. 전체 API
  **1,468 passed / 4 skipped**, 108.77초, Ruff·mypy 131개 소스 통과.
  판단 누락을 false로 채운 별도 대조군은 해당 경계 2개 실패, 17개 통과다.
- 최초 전체 검사의 임시 폴더 권한 오류를 기록하고 승인된 격리 환경에서 재검증했다.
  실제 문서·설정·대화 해시는 전후 동일하다. 신규 모델 39회 평가/설치/GUI 검증은 없다.
- 다음은 표 제목과 후속 행의 실제 좌표/순서 연결을 확인하고 합성 fixture로 검증하는 일이다.
  최종 답변 생성이나 앱 통합은 미완료다.
  [시험 구현·실패·한계](qa-evidence-selection-prototype-20260912.md),
  [익명 비교·보존 지표](qa-evidence-selection-prototype-20260912.json).

## 2026-09-12 분류 출처·누락 진단과 방어 회귀 — f7dfb5f

- 기준 2회와 비공개 프롬프트 대안별 2회, 총 8회 원래 이력 재생을 비교했다.
  기준 짧은 질문의 거부 claim 2개는 각각 수치 토큰 6개가 인용한 청크에 없었다.
  해당 수치는 이미 검색된 다른 청크에 있었으므로 검색 확대나 수치 가드 완화로 풀지 않는다.
- 기능등급 근거는 검색에 있었지만 기준 생성에서 빠졌다. 대안에서 생성되어도
  긴 질문은 어휘 거부, 짧은 질문은 최종 상충이 남았다. 세 대안 모두 제품에 반영하지 않았다.
  끝 표시 실험의 짧은 응답 기록 일부는 도구 출력에서 잘려 완전한 메타데이터만 사용했다.
- 합성 비의료 출처 경계 회귀 **8 passed**, 전체 API **1,411 passed / 4 skipped**,
  Ruff·mypy 130개 소스 통과. 전체 청크로 수치 검사를 대신한 별도 대조군은
  **4 failed / 4 passed**여서 잘못된 출처 우회에 대한 회귀 방어를 확인했다.
- 제품 프롬프트·모델·검색·검증·평가 게이트 변경 없음. 고정 전체 39회 평가는 재실행하지 않았다.
  실제 DB는 읽기 전용으로 열었으며 문서·설정과 Q&A 21/104/205행의 전후 해시가 같다.
  새 설치·GUI·사용자 대화 저장 없음. supported를 완전한 의미 근거 검증으로 간주하지 않는다.
- 다음 단위는 분류 항목별 원문 발췌와 출처를 먼저 검증하는 작은 시험 구현이다.
  [원인·미채택 대안·회귀·한계](qa-classification-source-diagnosis-20260912.md),
  [익명 진단·보존 지표](qa-classification-source-diagnosis-20260912.json).

## 2026-09-12 인용 경계의 상충 오탐 — dde86c6

- claim 전체의 설명·인용 어휘와 부정을 섞고 반복 주제어를 중복 집계한 오탐을 재현했다.
  인용 도입부별 구간을 비교하며 연결어·공통 은/는 형태 중복만 제외한다.
  인용 내용, 실제 상충·명시적 힌트·기존 근거 검증·평가 기준은 유지한다.
- 새 회귀 27개와 API **1,403 passed / 4 skipped**, Ruff·mypy 130개 소스 통과.
  기존 비교기의 별도 API 대조군은 오탐/미탐 2개 실패, 명시적 힌트 1개 통과다.
- 동일 실제 지원 claim 3개를 고정하면 conflicting_evidence → completed다.
  새 생성 두 건은 completed지만 각 1개만 지원되므로 분류 완결성 해결은 아니다.
  초기에 NYHA 표지 청크만 본 진단을 정정했다. I–IV 행 모두 검색되었고 답변에서 빠졌다.
- clean tree `dde86c6`, 동일 모델 digest의 첫 고정 전체 평가 **39/39, exit 0**,
  default_recommended. 상충 3/3, 안전 실패·변동 0, p50 2.329초 / p95 22.717초.
- 사용자 문서·설정·기존 Q&A 해시 보존. 설치·설정 변경·새 GUI 검증 없음.
  검색된 분류 근거의 답변 반영과 거부 claim의 수치/출처 연결을 다음에 진단한다.
  [변경·정정·회귀·다음 작업](qa-conflict-scope-20260912.md),
  [익명 재생·보존](qa-conflict-scope-20260912.json),
  [고정 전체 평가](qa-evaluation-20260912-conflict-scope.json).
