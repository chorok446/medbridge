...
> **출시 판정: 보류 (HOLD)** — 2026-09-10 대형 문서 Q&A 실패 확인
> `49131af` 합성 질문 10회 연속은 통과했으나 실제 431MB 문서에서 주제 혼동 오답이 나왔다.
> 검색 보강 `324e9fd` CI 설치본을 업데이트했다. 기존 데이터 보존과 재실행은 통과했지만
> 실제 정의 질문은 보류/목차 제목만 답하는 실패다. 전체 Windows 항목·대형 부하는 남는다.
> 고정 `74b9925`의 합성 모델 평가는 39/39 통과했으나 실제 대형 문서 실패를 대체하지 않는다.
