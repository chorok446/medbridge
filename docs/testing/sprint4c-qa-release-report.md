...
> **출시 판정: 보류 (HOLD)** — 2026-09-12 원문 묶음 선택 평가를 추가했다(앱 미연결).
> 두 분류 질문에서 관련 19블록·등급 4행을 보존했지만 합성 대조 3개 중 2개는 실패했다.
> 기준 누락·무관 자료 오선택이 남아 선택 품질 게이트는 통과하지 못했다.
> 새 단위 47개·추가 통합 1개, 관련 151개와 Ruff·mypy 통과. 실제 모델 총 11회, 평가 DB 쓰기 0회다.
> 최종 전체 API **1,738 passed / 4 skipped**, 120.77초. 자동 검사 통과와 선택 품질 미통과를 구분한다.
