...
> **출시 판정: 보류 (HOLD)** — 2026-09-17 하위 진단에서 관련 묶음의 other 오판정과 HTTP 500을 확인했다.
> 제한 진단은 1사례/3요청에서 중단됐다. 전체 품질 게이트 미실행이며 부분 결과를 통과로 처리하지 않는다.
> 기존 모델 실행 프로세스는 오류 후 조회되지 않았으나 적재 목록에는 모델이 남아 있었다. 원인은 미확정이다.
> 새 회귀 25개·관련 357개·전체 API **2,048 passed / 4 skipped**, Ruff·mypy 통과.
> 직전 HEAD ddad3f2의 CI `35116769854` 전체 success 확인. 새 평가 계측 커밋은 `c621ba5`다.
> 최신 제품 코드는 `9cc6414`이며 제품 동작·사용자 데이터·설치본에는 변경이 없다.
> 평가용 후보가 최종 답변이나 의미 검증 결과는 아니다. 실제 분류 답변 완결성은 미해결이다.
...
## 2026-09-17 하위 판단·전송 오류 진단 — c621ba5

- 평가 CLI의 opt-in 계측으로 검증된 target/basis/conflict와 전역 묶음 번호/단계를 남긴다.
  원문·경로·예외 메시지는 남기지 않으며 허용 예외 종류·숫자 코드만 제한적으로 보존한다.
- 계획은 mixed/sensor_axes 각각 최대 3회, 전체 최대 6사례/24요청, 최초 오류 중단이다.
  실제로는 첫 mixed에서 3요청 후 중단됐다. 전압과 단계 묶음의 other/type, other/stage를
  관찰했으며 세 번째 요청은 server_error였다. 최종 선택/전체 가드/다른 사례는 미측정이다.
- 서버 HTTP 500 시각은 00:53:20 KST다. 이후 llama-server PID 30940이 조회되지 않았으나
  `/api/ps`에는 모델이 남아 있었다. 적재 digest 일치를 생존/건강 증거로 승격하지 않는다.
  과거 연결 재설정의 원인이 같다는 근거와 이번 모델 실행 종료 이유는 아직 없다.
- 새 회귀 25개·관련 357개·전체 API 2,048개 통과/4개 건너뜀, Ruff·mypy 141개 소스 통과.
  전체 품질 게이트·제품 수정·사용자 데이터 접근·추가 재시작은 없다. Draft·HOLD 유지.
  [진단 계약·실측·한계](qa-step-diagnosis-20260917.md),
  [부분 판단과 실행 오류 기록](qa-step-diagnosis-20260917.json).

## 2026-09-17 묶음별 독립 선택 비교 — fbe0c35, 후보 미채택

- 독립 호출 뒤 전체 입력으로 상충/불확실성을 확인하는 평가 후보와 신규 합성 3사례를 동결했다.
  기존 지침/기대값·제품 동작은 유지했다. 호출별 재시도 금지와 공유 기한·취소·모델 검사를 보강했다.
- 대조군은 기존 자료 24회 완료·품질 실패 5회, 신규 자료 9회 완료·상태 누락 3회다.
  독립 후보는 기존 2/24회·신규 4/9회에서 각각 connect_failed로 중단됐다. 그전 과도한 보류와
  신규 자료의 상태 누락도 실패로 보존한다. 네 실행 모두 종료 2이며 새 후보를 채택하지 않는다.
- 총 사례 시도 39회·생성 요청 52회, 기대값 일치 27·품질 실패 10·실행 오류 2회다.
  새 회귀 74개·관련 332개·전체 API 2,023개 통과/4개 건너뜀, Ruff·mypy 140개 소스 통과.
- 서버 HTTP 200과 클라이언트 오류가 함께 관찰됐다. 원래 소켓 번호/하위 판단은 이번 집계에 없어
  보류 지점이나 과거 오류와의 동일 원인을 추정하지 않는다. 다음은 두 지점의 안전한 진단 계측이다.
- 사용자 DB/문서·설치본·Ollama 설정 변경 없음. 신규 3사례도 이제 개발에 노출된 자료다.
  [설계·결과·한계·후속 작업](qa-independent-selection-20260917.md),
  [전체 네 실행 집계](qa-independent-selection-20260917.json). Draft·HOLD 유지.

## 2026-09-17 상태 대상 해석 후보 비교 — be87c93, 후보 미채택

- 단독 진단에서 상태를 stage로 인식하지만 질문 대상과는 other로 판단하는 누락 지점을 확인했다.
- 실체와 상태 관계를 명확히 한 별도 후보를 동일 입력/스키마/파서/기대값으로 비교했다.
  기존 후보는 24회 중 같은 누락 3회, 새 후보는 무관 자료 오선택 후 3번째 연결 오류로 중단됐다.
- 새 후보는 기존 새 대상 사례에 도달하지 못했고, 이미 오선택도 관찰해 제품/기본값에 채택하지 않는다.
  개발에 쓴 8사례는 독립 검증 자료가 아니다. 다음은 별도 구조 대안과 새로운 자료를 비교한다.
- 총 시도 28회, 새 회귀 26개·관련 258개·전체 1,949개 통과/4개 건너뜀, Ruff·mypy 통과.
  Ollama 0.34.1에서도 클라이언트 오류가 있었으며 서버 200은 수신 성공의 근거가 아니다.
- 사용자 데이터·제품·설치본·Ollama 설정 변경 없음. Draft·HOLD 유지.
  [진단·미채택 근거·후속 작업](qa-entity-selection-20260917.md),
  [전체 집계](qa-entity-selection-20260917.json).

## 2026-09-15 대상·기준·이력·지시문 확장 평가 — f32b79f

- 모델 호출 전에 고정한 새 5사례를 기존 3사례와 함께 채점했다. 하나라도 실패/누락하면 통과하지 않는다.
- 새 대상의 구동 방식은 선택하지만 점검 상태를 누락하는 반례 2회를 검출했다. 나머지 새 네 사례는
  각각 두 반복에서 일치했으나 세 번째 반복 전 서버 오류로 중단돼 완료 판정하지 않았다.
- fixed는 4회 일치+1회 오류, extended는 16회 일치+2회 품질 실패+1회 오류다. 총 24회 시도이며
  결과 합산/재시도/기대값 조정/프롬프트 튜닝으로 실패를 덮지 않았다.
- Ollama 정상 시작 후 서버 HTTP 500 두 건과 자식 실행 프로세스 소멸을 관찰했다. `/api/ps` 목록은
  남아 있어 실행 생존의 충분한 근거가 아니다. 근본 원인은 미확정이며 추가 재시작·삭제는 없다.
- `분류해줘`의 호출 전 판별기 지원 제한도 기록했다. 제품 수정은 분리하며 이번 기대값은 그대로다.
- 새 회귀 65개·관련 232개·전체 1,923개 통과/4개 건너뜀, Ruff·mypy 138개 소스 통과.
  Draft·HOLD 유지. [계약·실패·후속 작업](qa-extended-selection-20260915.md),
  [전체 실행 집계](qa-extended-selection-20260915.json).

## 2026-09-14 연결 진단과 일반 모드 게이트 재검증 — 코드 4c20e16 유지

- 진단 54회에서 연결 재설정 7·HTTP 500 1회를 기록했다. 표준 소켓 생성과 조기 콜백 해제에서도
  실패하여 취소 가드 제거를 채택하지 않았다. 원래 재설정의 발생 주체/근본 원인은 미확정이다.
- 사용자 승인 후 상세 로그 모드 게이트는 9회 통과했다. 일반 모드 복구 중 부모만 종료해
  이전 모델 자식 두 개가 남은 에이전트 실수로 자원 경합이 발생했고, 첫 요청 시간 초과 두 건을 기록했다.
  해당 자식을 정리하고 메모리 해제·재적재한 최종 일반 모드 게이트도 9회 통과/종료 0이다.
- 현재 모델 실행 프로세스 하나, 상세 로그 비활성, 파일 삭제·설치/MedBridge 데이터 변경 없음.
  총 선택기 시도 74회, 관련 252개 통과, mypy 통과. 기존 코드 CI `34799820407` 전체 success다.
  새로운 대상·질문 한정·이력/지시문 혼합 사례 및 실제 문서 검증 전 제품 연결은 하지 않는다.
  [전체 진단·복구 절차·한계](qa-transport-diagnosis-20260914.md),
  [실패를 포함한 전송/게이트 집계](qa-transport-diagnosis-20260914.json).

## 2026-09-14 선택 후보 비교와 연결 진단 — 4c20e16

- 기본 `baseline`을 유지하고 지침 보강·대상/분류 정보 분리 판단 후보를 추가했다.
  고정 사례·정답표·전체 반복·실패 판정과 입력/원문 보존은 유지하며 모델 판단을 검증으로 승격하지 않는다.
- 기존 방식 9회 중 누락/오선택 6회, 지침 보강 9회 중 누락/상태 오판정 6회다.
  구조화 후보는 세 전체 실행의 부분 결과 12회가 기대값과 일치하지만 2·7·6번째 연결 오류로 중단됐다.
  모든 전체 실행은 종료 코드 2다. 별도 진단 1회 성공도 전체 통과에 포함하지 않는다.
- 오류 계측에서 `ConnectionResetError / WinError 10054`, 응답 헤더 수신 전 실패를 확인했다.
  서버 200 기록만으로 수신 성공을 주장하지 않는다. 다음은 재설정 원인 분리와 변경 없는 게이트 완주다.
- 새 회귀 65개·관련 167개·전체 1,858개 통과/4개 건너뜀, Ruff·mypy 통과다.
  직전 HEAD `4d177bf` CI `34798177790`는 확인 시 진행 중이며 새 코드 CI는 별도다.
  제품·사용자 데이터·설치본 변경 없음. PR Draft·출시 HOLD 유지.
  [비교·중단·진단·한계](qa-selection-comparison-20260914.md),
  [전체 실행과 오류 집계](qa-selection-comparison-20260914.json).

## 2026-09-14 출처 선택 회귀 평가 게이트 — c89d1d1

- 모델 밖의 고정 정답표로 필수 기준·정확한 출처·제외 자료·상태와 원문 보존을 검사한다.
  전체 3사례×3~5회 반복 중 한 번의 실패도 통과로 덮지 않는다. 미완료/미평가도 성공 종료하지 않는다.
  임의 문서의 의미 판별기나 출시 승인 로직은 아니며 일반 CI에 실제 모델 호출을 추가하지 않는다.
- 최초 실행은 7번째 평가 오류로 중단됐다. 오류 1건과 앞선 품질 실패 4건을 그대로 기록했다.
  진단 보강 뒤 최종 9회는 완료됐지만 누락 3·오선택 3회를 검출해 **종료 코드 2**를 확인했다.
  원인은 미확정이며 자동 재시도·선택 지시문 변경·사용자 문서/DB 접근은 없다.
- 새 단위 55개·관련 **124 passed**, Ruff·mypy 앱+실행기 136개 소스 통과다.
  최종 전체 API **1,793 passed / 4 skipped**, 132.97초다.
  이전 HEAD `ca69d2f` CI `34698274419` success 확인. 새 코드 CI는 별도다.
  다음은 이 고정 게이트를 유지하며 선택 방식을 개선·비교한다. Draft·HOLD 유지.
  [실행 계약·재현·종료 코드·한계](qa-selection-gate-20260914.md),
  [익명 집계](qa-selection-gate-20260914.json).

## 2026-09-12 원문 묶음 선택 평가 — b3f1cc8

- 확인된 청크 묶음을 전체 선택하고 원문을 합치거나 잘라 쓰지 않는다. 자동 그룹화·의미 검증은
  하지 않는다. 단일 청크 대조군도 지원한다. 원문·해시·좌표·순서·취소/기한·단일 호출을 보존한다.
- 기존 선택은 두 질문 모두 유형 5/9·단계 0/5블록을 선택한다. 새 대조군/묶음은 19블록을
  모두 유지하고 등급 행 4/4개가 일치한다. 평가자 지정 묶음이며 자동 관계 추론 성능은 아니다.
  긴 질문의 추가 3청크는 의미 검토가 남고, 지시문/표현 변경과 묶음 효과를 분리해 주장하지 않는다.
- 실제 모델 11회에는 최초 네트워크 오류 1회와 정상 합성 진단 1회가 포함된다. 같은 입력 재검사는
  정상이나 최초 원인은 미확정이다. 합성 기준 누락·지시문 포함 무관 자료 선택 **2개 실패**를 남겼다.
- 새 단위 47개·추가 PDF 통합 1개, 관련 **151 passed**, Ruff·mypy 134개 소스 통과다.
  최종 전체 API **1,738 passed / 4 skipped**, 120.77초다.
  직전 HEAD `0b2a914` CI `34697033754` success 확인. 이번 새 코드 CI는 별도다.
  문서 소속/묶음 확인·선택 완결성·관련성은 제품 연결 전에 별도 검증해야 한다. Draft·HOLD 유지.
  [계약·최초 실패·비교·적용 한계](qa-source-bundle-selection-20260912.md),
  [익명 집계](qa-source-bundle-selection-20260912.json).

## 2026-09-12 원문 보존 구조화 평가 — 0cbf176

- 모델 생성 대신 저장 청크의 정확한 문자 범위를 복사한다. 같은 계열의 연속 표지만 나누고
  불확실한 목록은 전체 원문을 유지한다. 머리말·조건·예외·뒤 문맥과 모든 출처를 보존한다.
  항목은 의미상 분류/조건 관계가 아니며 쪽수·bbox는 청크 전체다. 제품에는 연결하지 않는다.
- 새 단위 45개·합성 PDF 통합 1개, 전체 API **1,690개 통과 / 4개 건너뜀**, 160.68초.
  Ruff·mypy 133개 소스 통과다. PDF 실제 추출·검색·DB 재조회로 원문 보존과 답변 미저장을 확인한다.
- 기존 격리 사본의 긴/짧은 분류 질문에서 각 32개 청크·관련 19개 블록 출처를 모두 유지한다.
  기능등급 4개 항목은 두 질문 모두 원문 블록과 일치하고 전체 문자·해시·좌표 보존 오류는 0이다.
  유형·단계 구조는 미확정으로 전체 원문 유지. 관련성 선택·최종 분류 답변 완결성은 미검증이다.
- 모델 호출·평가 DB 쓰기 0회, 사용자 데이터·설치본·제품 코드 변경 없음. 새 39회·GUI는 미수행.
  직전 HEAD `783fcb2` CI `34696110383`는 확인 시 진행 중이며 이번 코드 CI는 별도다.
  다음은 표의 행·조건을 누락하지 않는 질문 관련 출처 묶음 선택 평가다. Draft·HOLD 유지.
  [보존 계약·재현·비교·적용 한계](qa-source-outline-20260912.md),
  [원문 비노출 집계](qa-source-outline-20260912.json).

## 2026-09-12 출처 고정 재서술 시험 — 37d55ab

- 단일 전체 원문·이력 없는 질문을 입력하고 모델에는 ID를 보내지 않는다. 서버가 출처를 고정한
  검토 후보와 거부 사유만 반환한다. 자동 선택·제품 공급자·라우트·대화 저장에는 연결하지 않는다.
- 새 회귀 61개, 관련 156개 통과. 수치·형식·취소·시간 상한과 원문/해시/좌표 보존을 검사했다.
  최종 전체 API 1,644개 통과/4개 건너뜀, 119.05초. Ruff·mypy 132개 소스 통과다.
  같은 청크 내부 수치 관계 뒤집힘은 기존 검사에서 잡지 못하는 한계로 명시했다.
- 동일 4개 출처 × 두 질문과 합성 사례 3개를 지침 전후 비교하고 실패 1건을 진단했다(총 23회).
  긴 질문은 4/4개 후보, 짧은 질문은 3/4개 후보로 수치 거부가 남는다. 제품 적용을 보류한다.
- 직전 `0d1f312` CI `34695045261`는 6개 작업 success. 새 평가 코드 CI는 별도다.
  [시험 계약·최초 실패·비교·실제 적용 한계](qa-source-bound-rewrite-20260912.md),
  [원문 비노출 집계](qa-source-bound-rewrite-20260912.json).

## 2026-09-12 전체 원문 인용의 출처 연결 복구 — 9cc6414

- 전체 원문과 유일하게 일치하는 짧은 청크만 연결을 복구한다. 조건/예외를 생략한 부분 인용,
  재서술·중복·미상 ID는 복구하지 않고 기존 수치·어휘·부정 검증도 계속 적용한다.
- 새 회귀 34개, 관련 74개, 전체 1,583개 통과/4개 건너뜀. 합성 PDF 응답·저장 출처와
  원문·해시·좌표 보존을 확인했다. 시험 자료의 병합/응답 필드 오류도 원인을 기록한 뒤 수정했다.
- 기존 긴/짧은 질문의 10개 생성 기록은 지원 2/4개·수치 거부 3/1개로 그대로다.
  이 좁은 수정으로 실제 분류 문제가 해결됐다고 판단하지 않는다. 후속은 재서술 출처 고정 평가다.
- 직전 HEAD `0644c4b` CI `34693308195` success 확인. 이번 코드 CI·고정 모델·GUI는 별도다.
  [복구 조건·최초 실패·오프라인 확인·미검증 범위](qa-literal-source-rebinding-20260912.md).

## 2026-09-12 전체 청크·검색·로컬 생성 비교 — 제품 변경 없음

- 1,060쪽 사본에서 저장 청크 16,239개와 단순 재생성의 내용·출처·검색 입력이 일치한다.
  읽기 순서 보정은 21쪽·82블록, 절 경계 표시는 37개다. 최종 청크는 16,270개다.
- 표/절 연결이 개선됐고 출처 31,401개의 누락·좌표·순서 불일치가 없다. 분류 관련 원문
  19블록은 전후 모두 모델 입력에 있었다. 정의 질문 대조군의 입력은 같다.
- 원래 이력 6/7턴과 같은 모델 digest로 긴/짧은 질문을 각각 전후 한 번씩 비교했다.
  지원 주장은 기존 3/3개 → 수정 사본 2/4개, 수치 거부는 수정 사본 3/1개다.
  거부된 숫자는 모두 다른 검색 청크에 있었으며 검증을 완화하거나 출처를 자동 보완하지 않았다.
- 논리 비교 4회/실제 7전송이다. 서버 오류 프레임 1건과 출처 형식 재요청 2건을 기록했다.
  네 응답의 completed 상태나 일부 표지 증가를 완전한 답변·출시 통과로 해석하지 않는다.
- 관련 회귀 130개 통과, 사용자 데이터 해시 동일. 생성 원문 상세 수동 평가는 보호 심사로
  미수행이며 원문 비노출 집계 검사만 수행했다. 새 39회·GUI·Windows 검증은 후속이다.
  [방법·실패·한계·다음 작업](qa-full-section-retrieval-20260912.md),
  [익명 측정](qa-full-section-retrieval-20260912.json).
- 제품 포함 HEAD `2e0bdff`의 CI `34692298914`는 success다. CI 성공이 위 생성 완결성
  실패나 미수행 실기기 항목을 대신하지 않으며 출시 HOLD를 유지한다.
