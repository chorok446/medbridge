...
> **출시 판정: 보류 (HOLD)** — 2026-09-10 사용자 설치 `7234601`에서 실제 431MB 자료의
> 정의 질문 3/3, 출처 이동·강조, 정상 재실행 후 이력 보존을 확인했다. 이어 `193babd`에서
> 상충 힌트에 지원 근거가 하나만 남으면 일반 완료 대신 보류하도록 고쳤다. API 1,340개와
> 새 고정 전체 모델 평가 39/39 통과. 이전 실패의 raw ID/final 힌트는 여전히 미확정이며
> 후속 상충 가드는 현재 설치본에 없다. 전체 Windows·대형 업로드/메모리 검증도 남는다.
