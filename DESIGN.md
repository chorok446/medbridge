...
- **Title** (600, 1.125rem/18px, lh 1.4): 카드 제목·문서명·섹션 헤더.
- **Body** (400, 0.875rem/14px, lh 1.625): 본문·안내문. *현재값이며, 읽기 패널은 상향 대상이다(아래 Rule 참조).*
- **Label** (500, 0.75rem/12px, lh 1.4): 배지·메타·버튼 보조. 12px 미만은 금지.

### Named Rules
**야간 가독 규칙.** 사용자가 실제로 '공부하는' 텍스트(추출 본문 읽기 패널)는 최소 17px + 행간 1.7을 목표로 한다. 14px 본문은 UI 안내문까지만 허용된다. PRODUCT.md: "40대 이상 사용자를 고려한 넉넉한 기본 글자 크기".
