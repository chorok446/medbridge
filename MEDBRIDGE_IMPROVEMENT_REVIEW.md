...
> - 최신 자동 검증일: 2026-09-17 (KST)
> - 기준 브랜치: `fix/summary-context-overflow`
> - 최초 검토 기준 커밋: `7d688b33e8a905311ebdc0d5a58feab0887c4663`
> - 마지막 제품 코드 검증 커밋: `9cc6414352a658492766aa103151dc3f27783509`
> - 마지막 테스트 검증 커밋: `c621ba59ad542832ab7aedc669c27b3f2cadbed3`
> - 마지막 평가 도구 검증 커밋: `c621ba59ad542832ab7aedc669c27b3f2cadbed3`
...
### 2026-09-17 최신 Q&A 진행

- `c621ba5`에서 평가 전용 `--diagnostics`를 추가했다. 원문 없는 하위 판단 범주/단계와
  허용 예외 종류·숫자 코드만 수집한다. 계측 on/off의 결과/요청 동등성과 비노출을 검증했다.
- 제한 진단은 첫 mixed 사례의 **3개 생성 요청**에서 중단됐다. 관련 전압 묶음은 other/type,
  진행 단계는 other/stage로 판단돼 제외됐다. 세 번째 요청은 서버 **HTTP 500**이었다.
  이전 최종 보류의 전체 경로와 sensor_axes는 미측정이며 전체 품질 게이트는 실행하지 않았다.
- 오류 후 기존 llama-server PID 30940은 조회되지 않았지만 `/api/ps`에는 모델이 남아 있었다.
  적재 목록을 생존 증거로 취급하지 않는다. 종료 원인과 과거 연결 재설정과의 관계는 미확정이다.
- 새 회귀 25개·관련 357개·전체 API **2,048 passed / 4 skipped**, Ruff·mypy 통과.
  직전 `ddad3f2`의 CI `35116769854` 전체 6개 작업 success를 확인했다.
- 다음은 모델 실행 프로세스 부재와 HTTP 500의 원인 분리다. 추가 생성/재시작/설정 변경 없이
  현재 증거를 보존했다. 선택 정책·제품·사용자 데이터·설치본 변경 없이 Draft·HOLD 유지.
  [계측 계약·부분 판단·서버 오류·후속 경계](docs/testing/qa-step-diagnosis-20260917.md).

### 2026-09-17 앞선 독립 선택 비교

- `fbe0c35`에서 묶음별 독립 호출+전체 상충/불확실성 가드 후보와 신규 3사례를 고정했다.
  공유 기한·취소·원문 보존·요청별 재시도 금지·각 응답의 모델 digest·실제 호출 수를 검증했다.
- 실제 비교 네 개는 **모두 실패**했다. 대조군은 기존 자료 24회 중 품질 실패 5회, 신규 자료
  9회 중 상태 기준 누락 3회다. 독립 후보는 기존 2/24회와 신규 4/9회에서 각각 연결 오류로
  중단됐으며, 그전에 과도한 보류와 상태 누락도 관찰해 기본값/제품에 채택하지 않는다.
- 총 생성 요청 52회·사례 시도 39회, 품질 실패 10회·실행 오류 2회를 보존했다.
  새 회귀 74개·관련 332개·전체 API **2,023 passed / 4 skipped**, Ruff·mypy 통과다.
  직전 `6389ba4`의 CI `35113956182` 전체 6개 작업 success를 확인했다.
- 다음은 선택 정책 변경 전에 원문 없는 하위 판단과 원래 연결 예외를 안전하게 계측하는 일이다.
  현재 집계만으로 보류 지점/전송 원인을 확정하지 않는다. 새 3사례도 관찰 후에는 개발 노출 자료다.
  사용자 데이터·제품·설치본·Ollama 설정 변경 없이 Draft·HOLD 유지.
  [설계·고정 계약·실패·후속 진단](docs/testing/qa-independent-selection-20260917.md).

### 2026-09-17 앞선 대상 해석 후보 비교

- `be87c93`에서 상태를 가진 실체의 해석을 명확히 한 비교 후보를 추가했다. 변경 없는 단독 진단에서
  모델이 점검 상태를 stage로 인식하면서 질문 대상은 other로 오판정하는 것을 확인했다.
- 현재 Ollama 0.34.1에서 기존 후보는 24회 완료·누락 3회, 새 후보는 **무관 출처 오선택 1회 후
  세 번째 요청 통신 오류로 중단**됐다. 새 반례에는 도달하지 못해 수정 효과도 미검증이다.
  새 후보는 기본값/제품에 채택하지 않고 명시적 평가 선택지로만 남긴다.
- 총 모델 시도 28회. 기존 8사례는 이번 지침 개발에 사용했으므로 개발용 회귀 자료로 구분한다.
  실패 후 지침·기대값 재조정/재시도 없음. 현재 버전에서도 연결 오류가 남아 원인 진단은 별도다.
- 새 회귀 26개·관련 258개·전체 API **1,949 passed / 4 skipped**, Ruff·mypy 통과.
  직전 CI `34921891931` 전체 success 확인. 사용자 데이터·제품·설치본 및 Ollama 설정 변경 없음.
- 다음은 지침 추가 대신 묶음별 독립 판단 등 선택 구조 대안과 새로운 독립 검증 자료의 비교다.
  Draft·HOLD 유지. [원인 범주·후보 미채택·실행 결과](docs/testing/qa-entity-selection-20260917.md).

### 2026-09-15 Q&A 진행

- `f32b79f`에서 기존 3사례를 포함한 **8사례 확장 평가**와 독립 기대값을 추가했다.
  새 회귀 65개·관련 232개·전체 API **1,923 passed / 4 skipped**, Ruff·mypy 통과다.
- 실제 모델 호출 전에 기대값과 코드를 고정했다. 새 대상의 점검 상태가 두 반복 모두 누락되어
  **선택 품질 실패 2회**를 검출했다. 다른 새 사례 네 가지는 각각 두 반복만 일치했다.
- 기존 fixed는 5/9회, 확장은 19/24회에서 HTTP 500으로 중단되어 **두 게이트 모두 미완료·실패**다.
  총 시도 24회이며 이전 성공이나 부분 통과를 합산하지 않는다. 모델 지침·제품 코드 변경은 없다.
- 시작 시 꺼져 있던 Ollama를 정상 시작했다. 오류 뒤 `/api/ps`는 모델을 보고하지만 관찰한 자식
  실행 프로세스가 사라졌으며 원인은 미확정이다. 추가 재시작·삭제·설정 변경은 하지 않았다.
- 자연어 `분류해줘`가 호출 전 판별기에서 거절되는 제한도 발견했다. 선택 평가용 질문만 모델 호출 전에
  같은 뜻의 지원 표현으로 조정했고, 해당 제품 개선은 별도다. 직전 CI `34801840531` 전체 success 확인.
- 다음은 고정 반례를 유지한 선택 후보 비교, 서버 오류 원인 분리, 자연스러운 요청 표현 지원이다.
  사용자 DB·설치본 변경 없음. Draft·출시 HOLD 유지.
  [확장 계약·누락 반례·서버 오류·검증 범위](docs/testing/qa-extended-selection-20260915.md).

### 2026-09-14 Q&A 진행

- `4c20e16` 코드와 고정 평가 조건을 그대로 두고 **일반 모드 3사례×3회 통과/종료 0**을 확인했다.
  사용자 승인으로 Ollama 상세 로그 재시작을 수행했고 마지막에는 일반 모드로 복구했다.
- 재시작 전 54회 전송 진단에서 WinError 10054 7회·HTTP 500 1회를 기록했다. 원래 연결
  재설정의 근본 원인은 미확정이며 취소 가드를 제거하거나 성공한 결과로 실패를 덮지 않았다.
- 재시작 중 부모만 종료한 에이전트 절차 실수로 이전 모델 실행 프로세스 두 개가 남았다.
  복구 후 두 시간 초과를 기록하고 해당 자식만 정리·모델 재적재했다. 최종 실행 프로세스 하나,
  상세 로그 비활성, 모델 digest 유지, 최종 게이트 9회 통과를 확인했다. 파일 삭제는 없다.
- 이번 선택기 호출 시도 74회, 관련 **252 passed**, mypy 통과. 제품/평가 코드·사용자 DB·설치본
  변경 없음. 직전 HEAD `466beb6` CI `34799820407` 전체 success 확인. Draft·출시 HOLD 유지.
  다음은 새로운 대상·질문 기준 한정·이력/지시문 혼합 사례 검증이다. 연결 오류·장기 안정성은 별도 미해결이다.
  [전송 진단·재시작 실수와 복구·최종 통과 범위](docs/testing/qa-transport-diagnosis-20260914.md).

### 2026-09-14 선택 후보 비교 기록

- `4c20e16`에서 **선택 후보 두 가지**를 기존 방식과 비교했다. 고정 정답표·게이트는 그대로다.
  지침만 보강하면 오선택은 줄지만 기준 누락·상태 오판정으로 **9회 중 6회 실패**한다.
- 대상/분류 정보를 별도로 판단하는 후보는 세 전체 실행에서 결과를 반환한 **12회가 기대값과
  일치**했다. 다만 각각 2·7·6번째 연결 오류로 중단되어 **완료/통과한 전체 실행은 없다**.
  중단 기록을 합쳐 성공으로 처리하지 않는다. 총 호출 시도 34회에는 대조군·단독 진단도 포함된다.
- 계측 실행에서 HTTP 응답 헤더 수신 전 `ConnectionResetError / WinError 10054`를 확인했다.
  Ollama 조회와 서버 200 기록은 정상이나 재설정 주체/근본 원인은 미확정이다. 재시도 우회는 없다.
- 새 회귀 65개·관련 **167 passed**, 전체 API **1,858 passed / 4 skipped**, 126.45초.
  Ruff·mypy 통과. 사용자 문서/DB 접근·제품/설치본 변경 없음. Draft·출시 HOLD 유지.
  직전 HEAD `4d177bf` CI `34798177790`는 확인 시 진행 중이며 새 커밋 CI는 별도다.
  다음은 연결 재설정 원인 분리 후 고정 게이트 완주와 별도 사례 검증이다.
  [비교·중단 기록·진단과 한계](docs/testing/qa-selection-comparison-20260914.md).

### 2026-09-14 고정 회귀 게이트 기록

- `c89d1d1`에서 **출처 선택 회귀 평가 게이트**를 추가했다. 고정 합성 3사례의 필수 기준·출처·제외 자료를
  모델 밖의 정답표로 대조한다. 전체 사례×3~5회 반복을 요구하고 누락·중복·실패·중단을 성공으로
  종료하지 않는다. 원문·좌표·조건 변조와 잘못된 상태도 검사한다. 제품/기존 선택기는 변경하지 않았다.
- 최초 7번째 요청 중단을 기록한 뒤 진단을 보강해 재검사했다. 최종 **9회 중 누락 3·오선택 3회**를
  검출하고 **종료 코드 2**를 확인했다. 총 선택기 호출 시도 16회이며 최초 오류 원인은 미확정이다.
  게이트 검출 기능은 확인했지만 선택 품질은 계속 미통과다. 임의 실제 문서의 의미 검증기가 아니다.
- 새 단위 55개·관련 **124 passed**, Ruff·mypy 통과. 사용자 문서/DB 접근·설치 변경 없음.
  최종 전체 API **1,793 passed / 4 skipped**, 132.97초다.
  이전 HEAD `ca69d2f` CI `34698274419` success 확인. 새 평가 코드 CI는 별도다.
  다음은 이 고정 게이트로 누락·오선택을 줄이는 선택 방식을 비교한다. Draft·출시 HOLD 유지.
  [판정 계약·실제 종료 코드·최초 오류·한계](docs/testing/qa-selection-gate-20260914.md).

### 2026-09-12 Q&A 진행 기록

- `b3f1cc8`에서 **원문 묶음 선택 평가**를 추가했다. 호출자가 연결을 확인한 청크는 함께 선택하되 원문을
  합치거나 일부 행만 남기지 않는다. 숨은 출처·불완전 분할·잘린 원문·전송 오류는 거부한다.
  자동 그룹화·관련성/완결성 인증·제품 연결은 없다. 기존 제품 `9cc6414`는 그대로다.
- 같은 격리 사본의 두 질문에서 기존 선택은 유형 5/9·단계 0/5블록이다. 새 단일 청크
  대조군과 묶음은 모두 **관련 19블록·기능등급 4행**을 보존했다. 다만 지시문/표현도 바뀌어
  개선을 묶음만의 효과로 주장하지 않는다. 긴 질문의 범위 밖 3청크는 의미상 필요성 미검증이다.
- 실제 모델 총 **11회**에 최초 네트워크 오류 1회와 진단 1회가 포함된다. 같은 입력 재검사는
  성공했으나 최초 오류 원인은 미확정이다. 합성 대조 3개 중 **기준 누락·무관 자료 오선택 2개 실패**다.
  선택 품질 게이트는 통과하지 못했으며 실패를 단위 테스트와 문서에 남긴다.
- 새 단위 47개·추가 PDF 통합 1개, 관련 **151 passed**, Ruff·mypy 통과다.
  최종 전체 API **1,738 passed / 4 skipped**, 120.77초다.
  직전 HEAD `0b2a914` CI `34697033754` success 확인. 새 평가 코드 CI는 별도다.
  평가 DB 쓰기·사용자 데이터/설치본 변경 없음. 다음은 기준 누락과 무관 자료 오선택을
  검출하는 평가 게이트다. PR Draft·출시 HOLD 유지.
  [선택 계약·실제 비교·합성 실패·한계](docs/testing/qa-source-bundle-selection-20260912.md).
- `0cbf176`에서 **원문 보존 구조화 평가**를 추가했다. 모델 생성 없이 연속 표지를 나누고
  표지·수치·조건·예외·뒤 문맥과 모든 검색 출처를 그대로 보존한다. 불명확하면 전체 원문을
  유지한다. 문자 구간을 의미상 검증된 분류나 항목별 좌표로 취급하지 않는다. 앱 미연결이다.
- 새 단위·통합 **46개**, 전체 **1,690 passed / 4 skipped**, Ruff·mypy 통과다.
  기존 격리 사본의 두 분류 질문에서 기능등급 **4개 행이 원래 표기로 원문 블록과 일치**하고,
  각 32개 검색 청크·관련 19개 블록의 출처와 전체 문자가 유지된다. 보존 오류 0건이다.
- 유형·단계 구간은 구조를 확정하지 못해 전체 원문으로 남겼다. 모델 호출·평가 DB 쓰기 0회,
  사용자 데이터·설치본·제품 `9cc6414` 변경 없음. 원문 보존과 최종 답변 완결성은 구분한다.
  직전 HEAD `783fcb2` CI `34696110383`는 확인 시 진행 중이며 이번 코드 CI는 별도다.
  다음은 표의 행·조건을 누락하지 않는 질문 관련 출처 묶음 선택 평가다. Draft·HOLD 유지.
  [보존 계약·합성 재현·격리 사본 대조·한계](docs/testing/qa-source-outline-20260912.md).
- `37d55ab`에서 **출처 고정 재서술 평가 경로**를 추가했다. 모델은 원문 하나만 보고
  ID는 서버가 고정한다. 검토 후보만 반환하며 앱에는 연결하지 않았다. 제품 `9cc6414`는 그대로다.
- 새 단위 61개·관련 156개·전체 **1,644 passed / 4 skipped**, Ruff·mypy 통과다.
  실제 모델 23회(두 비교 22 + 실패 진단 1)에서 긴 질문은
  4개 출처 모두 후보를 만들지만 짧은 질문은 수치 거부 1건이 남는다. 등급 표기 변환 징후를
  확인했으나 표기 보존 지침 보강만으로 해결되지 않았다. 기존 숫자 검사를 완화하지 않는다.
- 원문/출처·입력 스냅샷 보존, 격리 DB 읽기 전용, 사용자 데이터·설치본 변경 없음.
  직전 HEAD `0d1f312` CI `34695045261`는 모든 작업 success다. 신규 평가의 CI와 구분한다.
  다음은 등급 표지·수치·조건을 보존하는 구조화 후보 검증이며 제품 적용·출시는 계속 보류한다.
  [시험 구현·두 비교·실패 진단·한계](docs/testing/qa-source-bound-rewrite-20260912.md).
- `9cc6414`에서 짧은 **전체 청크 원문**과 유일하게 일치하는 인용의 실제 ID 오류만 복구한다.
  부분 인용·재서술·중복/미상 출처는 추측하지 않으며 기존 수치·어휘 검사를 유지한다.
- 새 단위/통합 **34개**, 관련 **74 passed**, 전체 **1,583 passed / 4 skipped**,
  Ruff·mypy 통과다. 합성 PDF의 실제 검색·응답·저장과 원문·해시·좌표 보존을 검증했다.
- 기존 격리 사본의 생성 기록 10개를 원문 비노출·읽기 전용으로 다시 검사했다.
  실제 수치 출처 오류 **4건은 그대로**이며 이번 좁은 복구 대상에 해당하지 않는다.
  신규 모델 호출·설치·사용자 데이터 변경 없음. 다음은 재서술 수치/조건의 출처 고정 평가다.
  직전 HEAD `0644c4b` CI `34693308195` success 확인. 새 코드 CI·고정 모델/GUI는 별도다.
  [재현·실패 기록·복구 조건·실제 적용 한계](docs/testing/qa-literal-source-rebinding-20260912.md).
- 제품 변경 없이 `5ef7e14`의 **1,060쪽 전체 격리 사본 비교**를 수행했다.
  청크는 저장 상태 16,239개 → 읽기 순서만 16,237개 → 절 경계 포함 16,270개다.
  원문 출처 31,401개의 누락·좌표/순서 불일치가 없고, 정의 질문 입력은 그대로다.
- 분류 원문 19블록은 전후 모두 검색됐다. 표/절 묶임은 개선됐지만 생성 단계의 출처 오류는 남는다.
  같은 질문 두 개를 전후 한 번씩 로컬 비교했다. 짧은 질문은 지원 표지가 늘었지만,
  긴 질문은 수치 거부 3건이다. 네 거부 주장의 빠진 숫자는 다른 검색 청크에는 존재했다.
- 실제 모델 비교 **4회 / 재시도 포함 7전송**, 관련 회귀 **130 passed**. 사용자 데이터 보존.
  제품 코드가 포함된 HEAD `2e0bdff`의 CI `34692298914`도 success로 완료됐다.
  생성 원문의 상세 수동 검토는 보호 심사로 미수행이며 집계 검사로 한계를 기록했다.
  다음은 합성 자료에서 생성 문장과 수치의 출처 연결을 재현·보강하는 일이다.
  [전체 비교·재시도·적용 한계](docs/testing/qa-full-section-retrieval-20260912.md).
- `5ef7e14`에서 검증된 두 줄 번호 절 경계를 추출 메타데이터로 저장하고 청크에 전달했다.
  짧고 같은 제목도 명시적 경계에서 다시 합치지 않는다. 일반 여러 줄 본문 규칙은 그대로다.
  새 회귀 **29개**, 관련 **130 passed**, 전체 **1,549 passed / 4 skipped**, Ruff·mypy 통과다.
- 실제 문제 페이지의 읽기 전용 메모리 비교에서 이전 표와 새 절을 분리하고 네 항목을
  함께 유지했다. 원문·출처·좌표와 사용자 데이터 전후 해시는 같다. 설치·모델 호출은 없다.
- 위 최신 항목에서 격리 사본의 전체 청크와 같은 질문을 비교했다. 기존 원본 자료에
  경계 표시가 없으므로 단순 청크 재생성만으로 적용되지 않는다. 원본 자동 재처리는 하지 않는다.
  최종 답변 완결성과 새 설치본 검증은 미완료이며 PR Draft·출시 HOLD를 유지한다.
  [검증된 절 경계 전달·대조군·남은 범위](docs/testing/qa-section-chunk-boundary-20260912.md).
...
- 실제 저장 좌표에서 문제의 두 블록 순서는 바로잡혔다. 다만 줄바꿈 절 제목을
  ...
  당시 후속인 절 경계 전달은 위 `5ef7e14`에서 수행했고 최종 답변 누락은 미해결이다.
  사용자 데이터·설치본은 그대로다.
