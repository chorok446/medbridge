...
def test_rejects_non_allowlist_model():
    with pytest.raises(ollama.ModelNotAllowedError):
        ollama.preflight("llama3:70b")


def test_skips_when_ollama_not_running(monkeypatch):
    monkeypatch.setattr(client, "get_status", lambda: client.OllamaStatus("not_running"))
    ready, reason = ollama.preflight("qwen3:8b")
    assert ready is False
    assert "실행" in reason


def test_skips_when_model_not_installed(monkeypatch):
...
def test_ready_when_installed(monkeypatch):
    monkeypatch.setattr(client, "get_status", lambda: client.OllamaStatus("ready", "0.9.0"))
    monkeypatch.setattr(
        client, "list_models",
