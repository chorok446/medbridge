...
keyring 호출은 OS 자격증명 저장소를 치는 동기 RPC라 전부 스레드로 위임한다 —
이벤트 루프에서 직접 부르면 지연 시 sidecar의 모든 API가 함께 멈춘다.
"""
...
async def test_connection(db: AsyncSession) -> tuple[bool, str]:
    ...
    try:
        ...
        # 블로킹 HTTP 호출(최대 CONNECTION_TEST_TIMEOUT_SEC)이라 스레드로 위임한다 —
        # 느린 endpoint 하나가 sidecar 전체를 얼리면 안 된다.
