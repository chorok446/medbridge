...
# 기존 후보는 대상을 상태/속성과 별개 실체로 오인했다. 응답을 강제로 교정하지 않고
# 대상 해석 지침만 분리 비교한다. 같은 스키마/검증 규칙을 쓰며 기본값은 바꾸지 않는다.
ENTITY_ASSESSMENT_SYSTEM = ASSESSMENT_SYSTEM + (
    " 대상 비교의 단위는 구분되는 실체다. 그 실체의 속성·기능·진행 상태를 설명하는 "
    "사실도 같은 실체에 대한 사실이다. 질문이 실체의 분류를 묻고 원문이 그 실체의 "
    "상태를 여러 범주로 구분한다면, 제목의 마지막 단어가 상태라는 이유로 target을 "
    "other로 판단하지 않는다. 어느 실체가 그 속성이나 상태를 갖는지 원문에서 확인한다. "
    "다른 실체의 속성·상태는 여전히 other다. 질문 단어가 언급되거나 선택 명령에 포함됐다는 "
    "것만으로 same으로 판단하지 않는다. 같은 실체라도 질문이 특정 기준만 요구하면 "
    "그 밖의 기준은 기존 규칙대로 basis none이다."
)
